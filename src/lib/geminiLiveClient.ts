...
function buildSystemInstruction(vaultContext: string): string {
    const base = `Ты — VoiceZettel, голосовой ассистент. Отвечай ТОЛЬКО на русском. Будь краток.`;

    if (vaultContext && vaultContext.trim().length > 0) {
        return `${base}

У тебя есть доступ к заметкам пользователя. Когда пользователь спрашивает о себе, своих предпочтениях, фактах — ищи ответ ниже и отвечай уверенно.

${vaultContext.slice(0, 12000)}`;
    }
